...
{
WaypointFollower::WaypointFollower() : nav2_util::LifecycleNode("WaypointFollower", "", false)
{
...
nav2_util::CallbackReturn WaypointFollower::on_configure(const rclcpp_lifecycle::State& /*state*/)
{
  ...
  gps_action_server_ = std::make_unique<ActionServerGPS>(
      ...
      get_node_waitables_interface(), "FollowGPSWaypoints",
...
std::vector<geometry_msgs::msg::PoseStamped> WaypointFollower::getLatestGoalPoses(const T& action_server)
{
  ...
  else
  {
    ...
    poses = convertGPSPosesToMapPoses(action_server->get_current_goal()->gps_poses);
  }
  return poses;
}
  ...
  while (rclcpp::ok())
  {
    ...
    if (new_goal)
    {
      ...
      send_goal_options.result_callback =
          std::bind(&WaypointFollower::resultCallback<rclcpp_action::ClientGoalHandle<ClientT>::WrappedResult>, this,
                    std::placeholders::_1);
...
template <typename T>
void WaypointFollower::resultCallback(const T& result)
{
  switch (result.code)
  {
...
WaypointFollower::convertGPSPosesToMapPoses(const std::vector<geographic_msgs::msg::GeoPose>& gps_poses)
{
