...
from nav2_msgs.action import FollowWaypoints, FollowGPSWaypoints, NavigateToPose
...
class BasicNavigator(Node):
    ...
    def __init__(self):
        ...
        self.follow_gps_waypoints_client = ActionClient(
            self, FollowGPSWaypoints, 'FollowGPSWaypoints')
    ...
    def followGPSWaypoints(self, gps_poses):
        ...
        while not self.follow_gps_waypoints_client.wait_for_server(timeout_sec=1.0):
            self.info(
                "'FollowGPSWaypoints' action server not available, waiting...")

        goal_msg = FollowGPSWaypoints.Goal()
        goal_msg.gps_poses = gps_poses

        self.info(f'Following {len(goal_msg.gps_poses)} goals....')
        ...
        if not self.goal_handle.accepted:
            self.error(
                f'Following {len(gps_poses)} waypoints request was rejected!')
            return False
